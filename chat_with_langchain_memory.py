...
class TokenLimitedChatMessageHistory(BaseChatMessageHistory):
    """自定义消息历史，基于 Token 数量自动截断"""
    ...
    def _trim_messages(self) -> None:
        """从最早的消息开始删除，直到 Token 总数不超过上限"""
        # 保留系统消息（如果有的话）—— 也可根据实际需求调整策略
        system_msg = None
        normal_msgs = []
        ...
        system_tokens = len(self.enc.encode(system_msg.content)) if system_msg else 0
        ...
        for msg in reversed(normal_msgs):
            ...
            if system_tokens + current_tokens + msg_tokens <= self.max_token_limit:
                kept_msgs.insert(0, msg)
                current_tokens += msg_tokens
            else:
                print(f"[System] 消息被遗忘以控制 Token 在 {self.max_token_limit} 以内")
                break
        
        self.messages = ([system_msg] if system_msg else []) + kept_msgs
...
def get_session_history(session_id: str) -> BaseChatMessageHistory:
    if session_id not in store:
        # 设置 Token 上限为 500（方便测试观察截断效果）
        store[session_id] = TokenLimitedChatMessageHistory(max_token_limit=500)
